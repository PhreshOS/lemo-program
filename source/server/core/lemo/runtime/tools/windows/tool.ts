...
const operations = {
    move: z.object({ action: z.literal("move"), ...coordinates, position }),
    resize: z.object({ action: z.literal("resize"), ...coordinates, size }),
    setGeometry: z.object({ action: z.literal("setGeometry"), ...coordinates, position, size }),
    minimize: z.object({ action: z.literal("minimize"), ...coordinates, minimized: z.boolean().optional() }),
    maximize: z.object({ action: z.literal("maximize"), ...coordinates, maximized: z.boolean().optional() }),
    changeTitle: z.object({ action: z.literal("changeTitle"), ...coordinates, title: z.string() }),
    raise: z.object({ action: z.literal("raise"), ...coordinates })
} satisfies { [Operation in keyof WindowOperations]: z.ZodType<{ action: Operation }> }

const input = z.discriminatedUnion("action", [
    z.object({ action: z.literal("inspect"), ...coordinates }),
