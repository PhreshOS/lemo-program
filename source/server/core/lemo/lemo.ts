...
export default class Lemo {
    ...
    public async tasks(): Promise<readonly Task[]> {
        ...
        return Object.freeze(await Promise.all(
            (await this.database.tasks()).map(async id => (await Task.open(this.database, this.memory, this.runtime, id))!)
        ))
    }
}
