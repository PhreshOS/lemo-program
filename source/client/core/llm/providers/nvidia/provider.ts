import type { NvidiaConfiguration } from "@server/core/llm/providers/nvidia/configuration"
...
/** One local configuration and Model handle for Nvidia. */
export default class NvidiaProvider implements LLMProvider {

    public readonly identity = "nvidia"
    public readonly name = "NVIDIA"

    private readonly retainedModels = new Map<string, NvidiaModel>()
    ...
    public async configure(configuration: NvidiaConfiguration): Promise<void> {
...
export const registration: LLMProviderRegistration = Object.freeze({
    identity: "nvidia",
    create: (models: LLMModelSource, source: LLMProviderSource) => new NvidiaProvider(models, source)
})
